...
public class LiveContestDriver {
    ...
    static void Load()
    {
        final long stopwatch = System.nanoTime();
		...
		solution =	new Solution(
						(BibTeXFile)loadFile(SourcePath),
//						(DocBook)loadFile(MutantPath),	// unused by this solution
						(ttc2019.live.docbook.DocBook)loadFile(TargetPath),
						(ModelChangeSet)loadFile(ChangePath)
					);

        LiveContestDriver.stopwatch = System.nanoTime() - stopwatch;
        Report(BenchmarkPhase.LoadModels, null);
    ...
    static void Transform() {
        ...
        stopwatch = System.nanoTime() - stopwatch;
        Report(BenchmarkPhase.Transform, null);
    }

    static void PostProcess() {
        stopwatch = System.nanoTime();

        solution.generateTraceabilityLinks();

        stopwatch = System.nanoTime() - stopwatch;
        Report(BenchmarkPhase.PostProcess, null);
    }
